...
function register(register_id, password, username, func) {
	isUntaken(username, function (str) {
		var gamePath = "ses_mine/home.html";
		var rejPath = "/form.html";
		if (str == "not taken") {
			var sql = "UPDATE players SET username=?, password=?, register=1 WHERE register_id=?;";
	...
	conn.query(sql, function (err, result, fields) {
		...
		(function asynLoop(i) {
			if (i >= result.length) {
				...
				var sql1 = "SELECT color FROM lobby_user WHERE username='" + result[i].username + "' and lobby_id=" + lobby_id + ";";
				conn.query(sql1, function (err, res, fields) {
					if (err) {
						console.log(sql1);
						throw err;
					}
					ret[i].color = res[0].color;
...
function putBets(jsonObject) {
    ...
    (function asyncLoop(i) {
		if (i >= bets.length) {
			...
			var bet = bets[i];
			bets[i].slots = bets[i].slots.toString();
			insert(table, bets[i]);
			asyncLoop(i + 1);
		}
	})(0);
...
function enterRouletteLobby(username, func, newLobbyFunc, startLobbyFunc) {
    ...
    sql = mysql.format(sql, preps);
    conn.query(sql, function (err, results, fields) {
        if (results.length == 0) {  //if the user is not present in any roulette lobby
...
function enterBingoLobby(username, lobby_id, func) {
	...
	conn.query(sql, function (err, results, fields) {
		...
		if(results[0] == null){	//if user not present in any lobby 
			...
			leaveBingoLobby(username, results[0].lobby_id);
		}
...
function updatePlayer(username, win, coins, coins_won, func) {  //if win win=1; else win=0 and coins_won=0
	if (win) {
		sql = "UPDATE players SET coins=coins+" + coins + ", coins_won=coins_won+" + coins_won + " where username=?;";
		preps = [username];
		sql = mysql.format(sql, preps);
		//        console.log(sql);
		conn.query(sql, function (err, result) {
		...
		sql = "UPDATE players SET coins=coins-" + coins + " where username=?;";
		preps = [username];
		sql = mysql.format(sql, preps);
		//       console.log(sql);
		conn.query(sql, function (err, result) {
